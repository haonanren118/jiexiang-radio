...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    /* 通过预设文件名（本地 m3u）添加/恢复内置源 */
    if (p === '/api/sources/preset' && req.method === 'POST') {
      const body = JSON.parse((await readBody(req)) || '{}');
      const file = body.file;
      const PRESETS = {
        'china-radio.m3u': { name: PRESET_NAME, local: PRESET_FILE },
        'qingting-radio.m3u': { name: QINGTING_SOURCE_NAME, local: QINGTING_FILE },
        'jiexiang-radio.m3u': { name: JIEXIANG_SOURCE_NAME, local: JIEXIANG_FILE },
        'fm-radio.m3u': { fm: true }
      };
      if (!PRESETS[file]) return sendError(res, 400, '未知预设源');
      let src;
      if (file === 'fm-radio.m3u') {
        src = ensureFmSource();
        await loadFmRadio(src);
      } else {
        const cfg = PRESETS[file];
        const id = idOf('src', 'preset:' + cfg.local);
        src = db.sources.find((s) => s.id === id);
        if (!src) {
          src = {
            id,
            name: (body.name || '').trim() || cfg.name,
            url: 'file://presets/' + cfg.local,
            local: cfg.local,
            builtin: true,
            enabled: true,
            count: 0,
      ...
      saveDB();
      return sendJSON(res, { source: src, sources: db.sources, stations: clientStations() });
    }
