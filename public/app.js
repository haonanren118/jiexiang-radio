...
(function () {
  ...
  function renderSources() {
    ...
    var mc = $('mine-src-count');
    if (mc) mc.textContent = state.sources.length ? t('src.countN', state.sources.length) : '';
  ...
  function sourceRow(src) {
    ...
    row.innerHTML =
      '<button class="src-caret" data-a="caret" title="' + esc(t('src.expand')) + '">▸</button>' +
      ...
      '</div>' +
      '<div class="src-detail" hidden></div>';

    row.querySelector('[data-a="caret"]').onclick = function (e) {
      e.stopPropagation();
      toggleSrcDetail(row, src);
    };
  ...
  /** 展开/收起某订阅源下的电台明细 */
  function toggleSrcDetail(row, src) {
    var det = row.querySelector('.src-detail');
    var caret = row.querySelector('.src-caret');
    if (det.hidden) {
      if (!det.dataset.loaded) { renderSrcDetail(det, src); det.dataset.loaded = '1'; }
      det.hidden = false; caret.textContent = '▾'; caret.title = t('src.collapse');
    } else {
      det.hidden = true; caret.textContent = '▸'; caret.title = t('src.expand');
    }
  }

  /** 渲染某订阅源下的电台列表（点击可播放） */
  function renderSrcDetail(det, src) {
    var list = (state.stations || []).filter(function (s) {
      return s.sourceId === src.id || s.sourceName === src.name;
    });
    if (!list.length) {
      det.innerHTML = '<div class="src-detail-empty">' + esc(t('src.detailEmpty')) + '</div>';
      return;
    }
    det.innerHTML = list.map(function (s) {
      return '<div class="src-detail-item" data-id="' + esc(s.id) + '">' +
        '<span class="di-name">' + esc(s.name) + '</span>' +
        (s.group ? '<span class="di-group">' + esc(s.group) + '</span>' : '') +
        '</div>';
    }).join('');
    Array.prototype.forEach.call(det.querySelectorAll('.src-detail-item'), function (it) {
      it.onclick = function () {
        var st = (state.stations || []).find(function (x) { return x.id === it.dataset.id; });
        if (st) play(st);
      };
    });
  }
  ...
  /** 首页「我的电台」随机 9 个（可刷新换一批） */
  var homeMineList = [];
  var homeMineBase = -1;
  function pickRandom(arr, n) {
    var pool = arr.slice(), i, j, tmp;
  ...
  function shuffleHomeMine() {
    homeMineList = pickRandom(state.stations, 9);
    homeMineBase = state.stations.length;
    renderHomeMine();
  }
  function renderHomeMine() {
    var grid = $('home-mine');
    grid.innerHTML = '';
    if (!homeMineList.length) { $('home-empty').hidden = false; return; }
    $('home-empty').hidden = true;
    homeMineList.forEach(function (st) { grid.appendChild(stationCard(st, { canDelete: true })); });
  }

  /** 首页：我的电台（随机 9 个，可刷新换一批）+ 最近收听横滑条 */
  function renderHome() {
    if (state.stations.length !== homeMineBase || !homeMineList.length) shuffleHomeMine();
    else renderHomeMine();
  ...
  function toggleSrcMenu(e) {
    if (e) e.stopPropagation();
    var pop = $('src-pop'); if (!pop || !current) return;
    if (!pop.hidden) { pop.hidden = true; pop.classList.remove('up'); return; }
    ...
    setTimeout(function () {
      ...
      if (spaceBelow < 0 && rect.top > rect.height + 20) pop.classList.add('up');
      else pop.classList.remove('up');
    }, 0);
  }

  function closeSrcMenu() {
    var pop = $('src-pop'); if (!pop || pop.hidden) return;
    pop.hidden = true; pop.classList.remove('up');
  }
  ...
  function bindIcons() {
    ...
    // 点击播放器源切换菜单外部关闭；点击按钮本身由 toggleSrcMenu 处理
    document.addEventListener('click', function (e) {
      var pop = $('src-pop'); if (!pop || pop.hidden) return;
      var target = e.target;
      if (target === sb || (sb && sb.contains(target)) || pop.contains(target)) return;
      closeSrcMenu();
    });
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape') closeSrcMenu();
    });
    ...
    var hmr = $('btn-home-mine-refresh');
    if (hmr) hmr.addEventListener('click', function () {
      if (!state.stations.length) { toast(t('toast.noStation')); return; }
      shuffleHomeMine(); toast(t('toast.shuffled'));
    });
  }
  ...
  function bindForms() {
    ...
    qa('[data-role="source-form"]').forEach(function (f) {
      ...
      var sel = f.querySelector('.preset-sel');
      if (sel) sel.addEventListener('change', function () {
        var urlEl = f.elements.namedItem('url');
        if (sel.value) urlEl.value = 'preset:' + sel.value;
  ...
  /** 通过预设文件名添加/恢复内置源 */
  function addPreset(file, name, f) {
    var btn = f ? f.querySelector('button[type="submit"]') : null;
    var old = btn ? btn.innerHTML : '';
    if (btn) { btn.disabled = true; btn.textContent = t('toast.srcLoading'); }
    post('/api/sources/preset', { file: file, name: name }).then(function (j) {
      if (j.sources) state.sources = j.sources;
      if (j.stations) state.stations = j.stations;
      renderAll();
      if (f) f.reset();
      toast(t('toast.presetAdded', j.source ? j.source.name : name));
    }).catch(function (err) {
      toast(I18N.msg(String(err.message)), true);
  ...
  function submitSource(f) {
    ...
    if (payload.url.indexOf('preset:') === 0) return addPreset(payload.url.slice(7), payload.name, f);
    if (!/^https?:\/\//i.test(payload.url)) return toast(t('toast.badUrl'), true);
    var btn = f.querySelector('button[type="submit"]');
    var old = btn ? btn.innerHTML : '';
    if (btn) { btn.disabled = true; btn.textContent = t('toast.srcLoading'); }
    post('/api/sources', payload).then(function (j) {
      if (j.sources) state.sources = j.sources;
        ...
        if (!state.sources.some(function (s) { return s.id === j.source.id; })) state.sources.push(j.source);
      }
      if (j.stations) state.stations = j.stations;
      renderAll(); f.reset();
